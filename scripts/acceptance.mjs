...
const entry = resolve(import.meta.dirname, '../examples/acceptance/sample/sse/Server.norm');
...
try {
  ...
  const response = await fetch(address + '/events', { headers: { Accept: 'text/event-stream' }, signal: AbortSignal.timeout(10000) });
  assert.equal(response.status, 200);
  assert.equal((await response.text()).split(/\r?\n/)[0], 'data: Norm SSE');
